                            ...
                            <table class="table" id="esacademic_table">
                                ...
                                <tbody>
                                    ...
                                    <tr class="tr-hover" role="button">
                                        ...
                                        <td class="{{ ($expertServiceAcademic->report_quarter == $currentQuarterYear->current_quarter && $expertServiceAcademic->report_year == $currentQuarterYear->current_year) ? 'to-submit' : '' }}" onclick="window.location.href = '{{ route('expert-service-in-academic.show', $expertServiceAcademic->id) }}' ">
                                        ...
                                        <td>
                                            <div class="btn-group" role="group" aria-label="button-group">
                                                ...
                                                <button type="button" value="{{ $expertServiceAcademic->id }}" class="btn btn-sm btn-danger d-inline-flex align-items-center" data-bs-toggle="modal" data-bs-target="#deleteModal" data-bs-esacademic="{{ $expertServiceAcademic->publication_or_audio_visual }}">Delete</button>
                                                @if ($submissionStatus[11][$expertServiceAcademic->id] == 0)
                                                    <a href="{{ url('submissions/check/11/'.$expertServiceAcademic->id) }}" class="btn btn-sm btn-primary d-inline-flex align-items-center">Submit</a>
                                                @elseif ($submissionStatus[11][$expertServiceAcademic->id] == 1)
                                                    <a href="{{ url('submissions/check/11/'.$expertServiceAcademic->id) }}" class="btn btn-sm btn-success d-inline-flex align-items-center">Submitted {{ $submitRole[$expertServiceAcademic->id] == 'f' ? 'as Faculty' : 'as Admin' }}</a>
