                            ...
                            <table class="table" id="mobility_table">
                                ...
                                <tbody>
                                    ...
                                    <tr class="tr-hover" role="button">
                                        ...
                                        <td class="{{ ($row->report_quarter == $currentQuarterYear->current_quarter && $row->report_year == $currentQuarterYear->current_year) ? 'to-submit' : '' }}" onclick="window.location.href = '{{ route('mobility.show', $row->id) }}' ">
                                            {{ $row->report_quarter }}
                                        </td>
                                            ...
                                            <div class="btn-group" role="group" aria-label="button-group">
                                                ...
                                                @if($row->classification_of_person == '298')
                                                    ...
                                                    @elseif ($submissionStatus[35][$row->id] == 1)
                                                        <a href="{{ url('submissions/check/35/'.$row->id) }}" class="btn btn-sm btn-success d-inline-flex align-items-center">Submitted as Designee</a>
                                                ...
                                                @else
                                                    @if ($submissionStatus[14][$row->id] == 0)
                                                        <a href="{{ url('submissions/check/14/'.$row->id) }}" class="btn btn-sm btn-primary d-inline-flex align-items-center">Submit</a>
                                                    @elseif ($submissionStatus[14][$row->id] == 1)
                                                        <a href="{{ url('submissions/check/14/'.$row->id) }}" class="btn btn-sm btn-success d-inline-flex align-items-center">Submitted {{ $submitRole[$row->id] == 'f' ? 'as Faculty' : 'as Admin' }}</a>
