...
@if (in_array(10, $roles))
    ...
    @forelse ( $departmentsResearch as $row)
        <a href="{{ route('reports.consolidate.research', $row->department_id) }}" class="submission-menu {{ isset($id) ? ($row->department_id == $id ||  request()->routeIs('reports.consolidate.research.$id') ? 'active' : '') : '' }}">
            {{ $row->code }} - Accomplishments
        </a>  
    @empty
    @endforelse
...
@if (in_array(11, $roles))
    ...
    @forelse ( $departmentsExtension as $row)
        <a href="{{ route('reports.consolidate.extension', $row->department_id) }}" class="submission-menu {{ isset($id) ? ($row->department_id == $id ||  request()->routeIs('reports.consolidate.extension.$id') ? 'active' : '') : '' }}">
            {{ $row->code }} - Accomplishments
        </a>  
    @empty
    @endforelse
